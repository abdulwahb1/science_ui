...
const FeatureCard = ({ title, icon, description, isActive }: FeatureCard) => {
  const cardClass = isActive ? "turn_purple" : "bg-white";
  return (
    <div
      className={`flex w-full flex-col items-start justify-between bg-custom-grey p-4 2xl:p-10 border rounded-2xl 2xl:h-[511px] ${cardClass}`}
    >
      <div className=" p-4  ">
        <Image
          src={icon}
          ...
          width={55}
          height={48}
          className="2xl:w-[76px] 2xl:h-[70px]"
        />
      </div>
